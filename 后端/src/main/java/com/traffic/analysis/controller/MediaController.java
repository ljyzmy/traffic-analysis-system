...
public class MediaController {
    ...
    /**
     * 从GridFS获取视频并支持范围请求，用于视频播放的快进和拖动功能
     * 
     * @param id GridFS文件ID
     ...
     * 从GridFS获取图像
     * 
     * @param id GridFS图像文件ID
     * @return 图像响应
     */
    @GetMapping("/image/{id}")
    public ResponseEntity<?> getImage(@PathVariable String id) {
        log.info("接收图像请求：ID={}", id);
        ...
        try {
            ...
            // 读取整个图像文件
            byte[] imageData = IOUtils.toByteArray(resource.getInputStream());
            
            log.info("返回图像文件: ID={}, 文件名={}, 大小={} bytes", 
                    id, file.getFilename(), imageData.length);
            ...
            log.error("获取图像文件失败: ID={}, 错误={}", id, e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("获取图像失败: " + e.getMessage());
        }
    }
} 